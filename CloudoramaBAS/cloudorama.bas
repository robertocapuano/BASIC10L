...
10 screen5,0:color 15,1,1:cls:keyoff:open"grp:"as#1:definta-z:R=rnd(-time):SH$="C=S;BU7 G5F5 E5H5":SX=90:SY=70:WC=12:WR=6:   HS$="C=S;U3 F5"
...
30 GN=4:UN=GN*2:MR=10:MC=10:remRND(1)*2+2: RND(0)*3
...
95 CO=0:CE=10:TU=1:
rem 100 PSET(35,20): print#1,"CREDIT";CE;"CO2";CO;"TURN";TU;
100 PSET(0,20): print#1,"THE ONLY WINNING MOVE IS NOT TO PLAY"
...
1090 ifk=32and(CS=0or((abs(CR-SR)+abs(CC-SC))<=2))then MU(CR,CC)=1: pset(X+2,Y+2): S=15:draw HS$:
1200 goto 1000
